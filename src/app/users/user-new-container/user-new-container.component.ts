...
export class UserNewContainerComponent implements OnInit {
  ...
  onSubmit(formData: IUserForm){
    const newUser = new UserModel(-1, formData.username, formData.email);
    newUser.info = new UserInfo();
    newUser.info.firstName = formData.firstName?formData.firstName: "";
    newUser.info.lastName = formData.lastName?formData.lastName: "";
    newUser.info.avatar = formData.avatar?formData.avatar: "";
